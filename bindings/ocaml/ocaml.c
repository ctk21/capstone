							...
							for (i = 0; i < lcount; i++) {
								switch(insn[j-1].detail->x86.operands[i].type) {
									...
									case X86_OP_IMM:
										tmp = caml_alloc(1, 2);
										tmp2 = caml_alloc(2, 0);
										Store_field(tmp2, 0, Val_int(insn[j-1].detail->x86.operands[i].imm));
										Store_field(tmp2, 1, Val_int(insn[j-1].detail->x86.encoding.imm_size));
									...
									case X86_OP_MEM:
										...
										Store_field(tmp2, 5, Val_int(insn[j-1].detail->x86.encoding.disp_size));
